...
public class RentACatImpl implements RentACat {
	...
	public String listCats() {
		// TODO - Done
		StringBuilder rentable = new StringBuilder(); //create stringbuilder object to hold all available cats
		//add if list is empty Rentable = ""
		if(cats == null || cats.size() == 0) {
			return "";
		}
		
		for(Cat c : cats) { 	//loop through every created cat
			if(c.getRented() == false) { //check to make sure it has not been rented
				rentable.append(c.toString()+"\n"); //if it hasnt been rented, add it. Otherwise, do nothing
			}
		}
		return rentable.toString(); //return stringbuilder in form of a string. Delimited by "\n"
	}
