...
public class BeanCounterLogicImpl implements BeanCounterLogic {
	...
	private Queue<BeanImpl>[] slots;
	private BeanImpl[] inFlightBean;
	private Queue<BeanImpl> remainingBeans;
	...
	BeanCounterLogicImpl(int slotCount) {
		// TODO: Implement
		inFlightBean = new BeanImpl[slotCount];
	...
	public int getInFlightBeanXPos(int yPos) {
		// TODO: Implement
		if (this.inFlightBean[yPos] == null) {
			return NO_BEAN_IN_YPOS;
		} else {
			return this.inFlightBean[yPos].getXPos();
		}
	}
	...
	private void insertTop() {
		if (this.inFlightBean.length == 0) {
			return;
		} 
		if (this.remainingBeans.size() > 0) {
			(this.inFlightBean[0] = this.remainingBeans.remove()).reset();
		} else {
			this.inFlightBean[0] = null;
		}
	}
	...
	public void reset(Bean[] beans) {
		...
		for (int i = 0; i < this.getSlotCount(); i++) {
			this.inFlightBean[i] = null;
		}
		
		if (beans != null) {
			for (int x = 0; x < beans.length; x++) {
				this.remainingBeans.add((BeanImpl) beans[x]);
			}
			this.insertTop();
		}
	...
	public boolean advanceStep() {
		...
		for (int i = this.getSlotCount() - 1; i >= 0; i--) {
			BeanImpl bean = this.inFlightBean[i];
			if (bean != null) {
				if (i == this.getSlotCount() - 1) {
					this.slots[bean.getXPos()].add(bean);
				} else {
					bean.choose();
					this.inFlightBean[i + 1] = bean;
				}
				ret =  true;
			} else if (i < this.getSlotCount() - 1) {
				this.inFlightBean[i + 1] = null;
			}
